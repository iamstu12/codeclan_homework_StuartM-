...
nyc_bikes_by_month <- nyc_bikes %>% 
  index_by(month) %>% 
  summarise(number_of_bike_hire_by_month = n()) 


nyc_bikes_by_month %>% 
  ggplot(aes(x = month, y = number_of_bike_hire_by_month)) +
  geom_line(group = 1)


```

The above graph has grouped every observation in 2018 into its respective month. I think this could be an okay way to forecast as it clearly shows the trend of the observations from month to month. Ideally, you would want more than one year however, which would make forecasting more difficult. With this data set you would be able to make a rough forecast for the following year for example, but that would be about the limit.
...
Now Summarise the number of bike hire counts by **date**. Make a plot of this new aggregated data. What does this plot tell you about the time series? Would this data be preferable for time series forecasting compared to the monthly data?
<br>
...
nyc_bikes_by_date <- nyc_bikes %>% 
  index_by(date) %>% 
  summarise(number_of_bike_hire_by_date = n()) 


nyc_bikes_by_date %>% 
  ggplot(aes(x = date, y = number_of_bike_hire_by_date)) +
  geom_line(group = 1)
...
The above graph has now grouped the observations by individual dates. This method, although looks messier, clearly displays the peaks and troughs of the observations recorded more clearly. The general trend is more or less the same
...
nyc_bikes_filled <- nyc_bikes_by_date %>%
  fill_gaps(number_of_bike_hire_by_date = as.integer(median(number_of_bike_hire_by_date)))

nyc_bikes_fit_dataset <- nyc_bikes_filled %>% 
  model(
    snaive = SNAIVE(number_of_bike_hire_by_date),
    mean_model = MEAN(number_of_bike_hire_by_date),
    arima = ARIMA(number_of_bike_hire_by_date)
  ) 

nyc_bikes_fit_dataset
...
nyc_bikes_4month_forecast <- nyc_bikes_fit_dataset %>% 
  fabletools::forecast(h = 120)

nyc_bikes_4month_forecast
...
nyc_bikes_4month_arima_forecast <- nyc_bikes_fit_dataset %>% 
  select(arima) %>% 
  fabletools::forecast(h = "4 months")

nyc_bikes_4month_arima_forecast
...
nyc_bikes %>% 
  ggplot(aes(x = date, y = start_lat, colour = month)) +
  geom_point() +
  facet_wrap(~bike_id)

nyc_bikes %>% 
  ggplot(aes(x = date, y = start_long, colour = month)) +
  geom_point() +
  facet_wrap(~bike_id)
